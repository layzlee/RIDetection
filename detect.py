...
def run(
    ...
    for frame_idx, batch in enumerate(dataset):
        ...
        if webcam:
            ...
            im = cv2.add(im, np.zeros(np.shape(im), dtype=np.uint8), mask=mask)  # 将两照片进行加和
            im = im.transpose((2, 0, 1))  # 一维三维互换
...
# Inference
        ...
        for i, det in enumerate(p):  # detections per image
            ...
            if webcam:  # batch_size >= 1
                patch, s, im0s, frame = path[i], f'{i}: ', im0s[
                    i].copy(), dataset.count  # path, im, im0s, vid_cap, s = batch
                ...
                im0 = cv2.addWeighted(im0s, 1, mask, 0.2, 0)
                cv2.polylines(im0s, [pts], True, (255, 255, 0), 3)
                # plot_one_box(dr, im0, label='Detection_Region', color=(0, 255, 0), line_thickness=2)
            else:
                patch, s, im0s, frame = path, '', im0s.copy(), getattr(dataset, 'frame', 0)
                cv2.putText(im0s, "Detection_Region", (int(im0s.shape[1] * wl1 - 5), int(im0s.shape[0] * hl1 - 5)),
                            cv2.FONT_HERSHEY_SIMPLEX,
                            1.0, (255, 255, 0), 2, cv2.LINE_AA)
                pts = np.array([[int(im0s.shape[1] * wl1), int(im0s.shape[0] * hl1)],  # pts1
                ...
                zeros = np.zeros((im0s.shape), dtype=np.uint8)
                mask = cv2.fillPoly(zeros, [pts], color=(0, 165, 255))
                im0s = cv2.addWeighted(im0s, 1, mask, 0.2, 0)
...
if __name__ == "__main__":
    ...
    main(opt)
            ...
            #hl1 = 560/1920  # 监测区域高度距离图片顶部比例
            #wl1 = 247 / 1080  # 监测区域高度距离图片左部比例
            #hl2 = 1262 / 1920  # 监测区域高度距离图片顶部比例
            #wl2 = 215 / 1080  # 监测区域高度距离图片左部比例
            #hl3 = 1550 / 1920  # 监测区域高度距离图片顶部比例
            #wl3 = 765 / 1080  # 监测区域高度距离图片左部比例
            #hl4 = 499 / 1920  # 监测区域高度距离图片顶部比例
            #wl4 = 5894 / 1080  # 监测区域高度距离图片左部比例
            #if webcam:
                #for b in range(0, im.shape[0]):
